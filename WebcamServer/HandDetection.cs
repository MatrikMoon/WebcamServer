...
public class HandDetection
{
    public void DetectHands()
    {
        ...
        while (true)
        {
            ...
            float aspectRatio = capture.FrameWidth / (float)capture.FrameHeight;

            int inHeight = 300;
            int inWidth = ((int)(aspectRatio * inHeight) * 8) / 8;

            using var net = CvDnn.ReadNetFromCaffe(modelTxt, model);
            using var inpBlob = CvDnn.BlobFromImage(image, 1.0 / 255, new Size(inWidth, inHeight), new Scalar(0, 0, 0), false, false);
            ...
            for (int n = 0; n < nPoints; n++)
            {
                ...
                Cv2.Resize(probMap, probMap, new Size(capture.FrameWidth, capture.FrameHeight));
                Cv2.MinMaxLoc(probMap, out _, out var maxVal, out _, out var maxLoc);

                if (maxVal > thresh)
                {
                    Cv2.Circle(frameCopy, maxLoc.X, maxLoc.Y, 8, new Scalar(0, 255, 255), -1, LineTypes.Link8);
                    Cv2.PutText(frameCopy, Cv2.Format(n), new OpenCvSharp.Point(maxLoc.X, maxLoc.Y), HersheyFonts.HersheyComplex, 1, new Scalar(0, 0, 255), 2, LineTypes.AntiAlias);
                }
            ...
            for (int n = 0; n < nPairs; n++)
            {
                ...
                Cv2.Line(image, partA, partB, new Scalar(0, 255, 255), 8);
                Cv2.Circle(image, partA.X, partA.Y, 8, new Scalar(0, 0, 255), -1);
                Cv2.Circle(image, partB.X, partB.Y, 8, new Scalar(0, 0, 255), -1);
            }
